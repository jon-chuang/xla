...
FailureOr<linalg::MatmulOp> rewriteBatchMatmulAsMatmul(
    linalg::BatchMatmulOp bmOp, PatternRewriter &rewriter) {
...
struct BatchMatmulOpTransformPattern
  ...
  LogicalResult matchAndRewrite(linalg::BatchMatmulOp bmOp,
    ...
    rewriter.replaceOp(bmOp, tilingResult->replacements);

    auto tiledBm = cast<linalg::BatchMatmulOp>(tilingResult->tiledOps.front());
    return rewriteBatchMatmulAsMatmul(tiledBm, rewriter);
  }
};
