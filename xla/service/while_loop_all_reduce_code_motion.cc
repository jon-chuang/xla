...
//     reduce-scatter)
//  2. Do not allow any forwarding instructions like convert.
//  3. Allow select only when the predicate is effectively scalar.
      ...
      *param_body->mutable_shape() = ShapeUtil::MakeTupleShape(element_shapes);
      ...
      while (!reduce_scatter_users.empty()) {
        ...
        if (user->opcode() == HloOpcode::kSelect) {
          // Select should a zero on one side and reduce-scatter on other
  ...
  while (run_next_pass) {
    ...
    for (HloComputation* computation :
      ...
      for (HloInstruction* while_body_instruction :
        ...
        if (op != HloOpcode::kAllReduce && op != HloOpcode::kReduceScatter) {
          continue;
        }
